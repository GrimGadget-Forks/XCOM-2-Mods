//---------------------------------------------------------------------------------------
//  FILE:   XComDownloadableContentInfo_VeteranTrainingDefenseMatrixEdition.uc                                    
//           
...
class X2DownloadableContentInfo_VeteranTrainingDefenseMatrixEdition extends X2DownloadableContentInfo;
...
static function UpdateRMOTSTemplates(XComGameState StartState)
{
	...
	for( DifficultyIndex = `MIN_DIFFICULTY_INDEX; DifficultyIndex <= `MAX_DIFFICULTY_INDEX; ++DifficultyIndex )
	{
		...
		if(Template == none) 
		{
			`Redscreen("RM Veteran Training: Failed to find facility template UFODefense difficult=" $ DifficultyIndex);
			continue;
		}
		...
		if(Template.StaffSlots.Length == 1) 
		{
			...
			`log("RM Veteran Pack: Added OTSVeteranSlot to facility template UFODefense");
