//---------------------------------------------------------------------------------------
//  FILE:   XComDownloadableContentInfo_PartiallyGifted.uc                                    
//           
...
class X2DownloadableContentInfo_PartiallyGifted extends X2DownloadableContentInfo;
...
static function UpdateRMOTSTemplates(XComGameState StartState)
{
	...
	`Log("RM Psi Veteran Training : Updating Psi Lab Templates");
	...
	for( DifficultyIndex = `MIN_DIFFICULTY_INDEX; DifficultyIndex <= `MAX_DIFFICULTY_INDEX; ++DifficultyIndex )
	{
		...
		if(Template == none) 
		{
			`Redscreen("RM Psi Veteran Training: Failed to find facility template UFODefense difficult=" $ DifficultyIndex);
			continue;
		}
		...
		if(Template.StaffSlots.Length == 3) 
		{
			Template.StaffSlots.AddItem('PsiVeteranSlot');
			Template.StaffSlotsLocked = 1;
			`log("RM Veteran Pack: Added OTSVeteranSlot to facility template PsiChamber");
...
//	{
//		`log("RM Veteran Slot: OTS had only single staff slot, attempting to update facility"); 
