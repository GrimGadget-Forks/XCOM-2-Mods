...
class UIScreenListener_Facility_PsiLab_PsiVeteranSlot extends UIScreenListener;

var UIButton OfficerButton;
//var UIFacility_Academy ParentScreen;
var UIFacility_RM_PsiVeteranSlot Slot;
...
event OnInit(UIScreen Screen)
{
	...
	local UIFacility_PsiLab ParentScreen;
	local XComGameState NewGameState;
	local XComGameStateHistory History;
	local name TemplateName;
	local XComGameState_FacilityXCom FacilityState, OTSState;

	`Log("RM Veteran Training : Searching for existing Psi LabFacility");
	TemplateName = 'PsiChamber';
	History = `XCOMHISTORY;
	...
	ParentScreen = UIFacility_PsiLab(Screen);
	...
	class'X2DownloadableContentInfo_PartiallyGifted'.static.OnLoadedSavedGame();
	...
	ParentScreen.m_kStaffSlotContainer = ParentScreen.Spawn(class'UIFacility_StaffContainer_RM_PsiLab', ParentScreen);
...
{
	//UpdateStaffSlots();
	UIFacility_PsiLab(Screen).m_kStaffSlotContainer.Show();
}
// This event is triggered after a screen loses focus
event OnLoseFocus(UIScreen Screen)
{
	//UpdateStaffSlots();
	UIFacility_PsiLab(Screen).m_kStaffSlotContainer.Hide();
}
...
static function CreateStaffSlots(XComGameState_FacilityXCom FacilityState, XComGameState NewGameState)
{
	...
	for (i = FacilityState.StaffSlots.Length ; i < FacilityTemplate.StaffSlots.Length; i++)
	{
		...
		StaffSlotTemplate1 = X2StaffSlotTemplate(class'X2StrategyElementTemplateManager'.static.GetStrategyElementTemplateManager().FindStrategyElementTemplate('PsiVeteranSlot'));
