...
//---------------------------------------------------------------------------------------
//  FILE:    UIFacility_RM_VeteranSlot.uc
//  AUTHOR:  RealityMachina (using Long War Studio's Officer Pack as a base)
//           
//  PURPOSE: Reworked UIFacility_StaffSlot for veteran functionality
//---------------------------------------------------------------------------------------

class UIFacility_RM_VeteranSlot extends UIFacility_StaffSlot dependson(UIPersonnel) config(RM_VeteranSlot);


var localized string m_strTrainRMVeteranDialogTitle;
var localized string m_strTrainRMVeteranDialogText;
var localized string m_strStopTrainRMVeteranDialogTitle;
var localized string m_strStopTrainRMVeteranDialogText;
...
simulated function OnClickStaffSlot(UIPanel kControl, int cmd)
{
	...
	case class'UIUtilities_Input'.const.FXS_L_MOUSE_UP_DELAYED:
		...
		else // Ask the user to confirm that they want to empty the slot and stop training
		{
			...
			StopTrainingText = m_strStopTrainRMVeteranDialogText;
			StopTrainingText = Repl(StopTrainingText, "%UNITNAME", UnitState.GetName(eNameType_RankFull));

			ConfirmEmptyProjectSlotPopup(m_strStopTrainRMVeteranDialogTitle, StopTrainingText);
		}
		break;
...
simulated function OnVeteranTrainSelected()
{
	...
	DialogData.strTitle = m_strTrainRMVeteranDialogTitle;
	DialogData.strText = m_strTrainRMVeteranDialogText;
...
simulated function OnPersonnelSelected(StaffUnitInfo UnitInfo)
{
	...
	DialogData.eType = eDialog_Alert;
	DialogData.strTitle = m_strTrainRMVeteranDialogTitle;
	DialogData.strText = `XEXPAND.ExpandString(m_strTrainRMVeteranDialogText);
	DialogData.strAccept = class'UIUtilities_Text'.default.m_strGenericYes;
	DialogData.strCancel = class'UIUtilities_Text'.default.m_strGenericNo;
...
{
	local UIPersonnel_RMVeteran kPersonnelList;
	...
	if (eAction == eUIAction_Accept)
	{
		...
		{
			kPersonnelList = Spawn( class'UIPersonnel_RMVeteran', HQPres);
...
simulated function OnSoldierSelected(StateObjectReference Unit)
{
	...
	StaffSlotState = `XCOMHQ.GetFacilityByName('OfficerTrainingSchool').GetStaffSlot(1);
